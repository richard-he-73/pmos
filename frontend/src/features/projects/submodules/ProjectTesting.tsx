...
const ProjectTesting: React.FC = () => {
  ...
  const { data: testCases = [], isLoading: loading } = useGetTestCasesQuery({ project_id: projectId });
  const { data: defects = [] } = useGetDefectsQuery({ project_id: projectId });
  const { data: reports = [] } = useGetTestReportsQuery({ project_id: projectId });
